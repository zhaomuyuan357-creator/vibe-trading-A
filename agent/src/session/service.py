...
class SessionService:
    ...
    def create_session(self, title: str = "", config: Optional[Dict[str, Any]] = None, user_id: str = "local") -> Session:
        """Create a new session.

        Args:
            title: Session title.
            config: Session configuration.
            user_id: Product workspace owner.

        Returns:
            The newly created Session.
        """
        session = Session(title=title, config=config or {}, user_id=user_id or "local")
    ...
    def list_sessions(self, limit: int = 50, user_id: Optional[str] = None) -> list[Session]:
        """List all sessions."""
        sessions = self.store.list_sessions(limit=limit if user_id is None else max(limit * 5, limit))
        if user_id is not None:
            sessions = [session for session in sessions if (session.user_id or "local") == user_id]
    ...
    async def _run_attempt(
        ...
        *,
        ...
        try:
            ...
            if attempt.run_dir:
                self._write_run_owner(Path(attempt.run_dir), session.user_id, session.session_id)
    ...
    @staticmethod
    def _write_run_owner(run_dir: Path, user_id: str, session_id: str) -> None:
        """Persist workspace ownership beside generated run artifacts."""
        try:
            run_dir.mkdir(parents=True, exist_ok=True)
            (run_dir / "owner.json").write_text(
                json.dumps(
                    {"user_id": user_id or "local", "session_id": session_id},
                    ensure_ascii=False,
                    indent=2,
                ),
                encoding="utf-8",
    ...
    async def _run_with_agent(
        ...
        *,
        ...
        llm = ChatLLM(llm_settings=llm_settings if isinstance(llm_settings, dict) else None)
        pm = PersistentMemory()
