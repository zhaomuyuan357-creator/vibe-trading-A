...
def _setting(settings: Mapping[str, str] | None, name: str, default: str = "") -> str:
    ...
    return str(settings.get(name, default) or "")
...
def build_llm(
    *,
    ...
    if settings is None:
        _sync_provider_env()
    name = model_name or _setting(settings, "LANGCHAIN_MODEL_NAME", "").strip()
    if not name:
    ...
    if provider in {"openai-codex", "openai_codex"}:
        ...
        effort = _setting(settings, "LANGCHAIN_REASONING_EFFORT", "").strip().lower()
        return OpenAICodexLLM(
            model=name,
            temperature=temperature,
            timeout=int(_setting(settings, "TIMEOUT_SECONDS", "120")),
            reasoning_effort=effort or None,
        )

    if provider == "deepseek" and settings is None:
    ...
    api_key = _setting(settings, key_env or "", "") if settings is not None and key_env else ""
    ...
    if caps.default_headers:
        ...
        if caps.name == "moonshot":
            custom_ua = _setting(settings, "MOONSHOT_USER_AGENT", "").strip()
            if custom_ua:
