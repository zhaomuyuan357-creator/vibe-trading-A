...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  const mergedHeaders: Record<string, string> = { "Content-Type": "application/json; charset=utf-8", ...authHeaders() };
  if (headers) {
...
export const api = {
  login: (email: string, access_code: string) =>
  ...
  listWhitelist: () => request<WhitelistEntry[]>("/auth/whitelist"),
  upsertWhitelist: (body: UpsertWhitelistRequest) =>
    request<WhitelistEntry>("/auth/whitelist", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  setWhitelistStatus: (email: string, status: "active" | "disabled") =>
    request<WhitelistEntry>(`/auth/whitelist/${encodeURIComponent(email)}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    }),
  getDeploymentReadiness: () => request<DeploymentReadinessResponse>("/deployment/readiness"),
  uploadFile,
  ...
  analyzeSingleStock: (code: string, lookback = 120) =>
    request<SingleStockAnalysis>(
      `/single-stock/analyze?code=${encodeURIComponent(code)}&lookback=${encodeURIComponent(String(lookback))}`,
    ),
  saveSingleStockAnalysis: (analysis: SingleStockAnalysis) =>
    request<SingleStockAnalysisRecord>("/single-stock/analyses", {
      method: "POST",
      body: JSON.stringify({ analysis }),
    }),
  listSingleStockAnalyses: (symbol?: string, limit = 20) => {
    const q = new URLSearchParams();
    if (symbol) q.set("symbol", symbol);
    q.set("limit", String(limit));
    return request<SingleStockAnalysisRecord[]>(`/single-stock/analyses?${q.toString()}`);
  },
  getSingleStockAnalysis: (analysisId: string) =>
    request<SingleStockAnalysisDetail>(`/single-stock/analyses/${encodeURIComponent(analysisId)}`),
  ...
  getChannelConfig: () => request<ChannelConfigResponse>("/channels/config"),
  updateChannelConfig: (channel: string, body: UpdateChannelConfigRequest) =>
    request<ChannelConfigResponse>(`/channels/config/${encodeURIComponent(channel)}`, {
      method: "PUT",
      body: JSON.stringify(body),
    }),
...
export interface SingleStockSummary {
  latest_close: number;
...
export interface SingleStockFactorRow {
  name: string;
  weight: number;
  score: number;
...
export interface SingleStockLevelProbability {
  name: string;
  price: number;
  basis: string;
  distance_pct: number;
  touch_probability_5d: number;
  touch_probability_10d: number;
  model: string;
}

export interface SingleStockEvidence {
  label: string;
  source: string;
  status: "verified" | "model_calculation" | "not_connected_v1" | string;
  detail: string;
  as_of: string;
  url?: string;
}

export interface SingleStockSupplementalSection {
  status: string;
  source: string;
  url?: string;
  summary?: string;
...
export interface SingleStockAnalysis {
  ...
  factors: {
    ...
    formula?: string;
    rows: SingleStockFactorRow[];
  };
  level_probabilities: SingleStockLevelProbability[];
  evidence: SingleStockEvidence[];
  supplemental?: Record<string, SingleStockSupplementalSection>;
  warnings: string[];
  disclaimer: string;
}

export interface SingleStockAnalysisRecord {
  id: string;
  symbol: string;
  name: string;
  as_of: string;
...
export interface SingleStockAnalysisDetail extends SingleStockAnalysisRecord {
  payload: SingleStockAnalysis;
}
...
export interface UpsertWhitelistRequest {
  email: string;
  role?: "user" | "admin";
  note?: string;
}
...
export interface DeploymentSummary {
  ready_for_public_deploy: boolean;
